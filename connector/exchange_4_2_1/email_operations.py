...
from exchangelib import Message, HTMLBody, FileAttachment, Mailbox, EWSDateTime, EWSTimeZone, Q
...
from .exchange_const import MESSAGES, DEFAULT_FOLDER_PATH, MAX_RETRY, DELAY_TIME, EXTRACT_EMAIL_METADATA_LEGACY
...
def upload_file_to_cyops(file_name, file_content, file_type=None):
    try:
        ...
        file_id = response['@id']
        file_description = 'Email file as attachment'
...
def process_email_data(unread_emails, params, folder_path, updated_headers, **kwargs):
    try:
        ...
        for email in unread_emails:
            ...
            parsed_email = explode_email(raw_msg, parse_inline, extract_attach_data, updated_headers, **kwargs)
            parsed_email.update({'folder_path': folder_path})
            parsed_email['item_id'] = email.id
            if save_as_attachment:
                attach_response = upload_file_to_cyops('email_as_attch.eml', raw_msg)
...
def parse_query_result(src, qs, pull_oldest, limit, parse_inline, extract_attach_data, exclude_absolute_path, **kwargs):
    emails = []
    ...
    for email in qs:
        ...
        parsed_email = explode_email(raw_msg, parse_inline, extract_attach_data,
                                     updated_headers=EXTRACT_EMAIL_METADATA_LEGACY, **kwargs)
        parsed_email['item_id'] = email.id
        if exclude_absolute_path:
        ...
        if query_method == 'Advanced':
            ...
            if query:
                ...
                if 'datetime_received__gt' in query.keys():
                    ...
                    localized_dt = tz.localize(
                        EWSDateTime(dt_obj.year, dt_obj.month, dt_obj.day, dt_obj.hour, dt_obj.minute, dt_obj.second))
        ...
        for folder in mailbox_folder:
            ...
            if qs.count():
                emails.extend(parse_query_result(folder, qs, pull_oldest, limit, parse_inline,
                                                 extract_attach_data, exclude_absolute_path, **kwargs))
