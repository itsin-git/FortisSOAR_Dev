    ...
    if file_name:
        ...
        if file_type == 'XML':
            ...
            if file_name.endswith('.xlsx') or abs_filename.endswith('.XLSX'):
                ...
                wb = pd.read_excel(abs_filename, na_filter=False, sheet_name=None)
                data = OrderedDict()
                for sheet, sheet_content in wb.items():
                    sheet_data = []
                    title_row = sheet_content.keys().values
                    for row in sheet_content.values:
                        row_data = {}
                        row_values = list(row)
                        for cell_index in range(0, len(title_row)):
                            row_data[title_row[cell_index]] = row_values[cell_index]
