...
def create_file_from_string(contents, filename=None, mimetype='text/plain', *args, **kwargs):
    """
    Creates a (temporary) file filled with `contents`. The default content-type of this
    ...
    metadata = FileMetadata(filename, os.path.getsize(path), mimetype,
...
def download_file_from_cyops(iri, headers=None, *args, **kwargs):
    ...
    if obj_type != 'files':
        ...
        if not isinstance(file_iri, str):
