...
def my_iter_attachments(eml, level_count):
    ...
    def is_attachment(part):
        ...
        if c_d is None:
            return False
        # inline images in .eml should be considered attachments
        if c_d.split(';')[0] == 'inline':
            return True
    ...
    for part in parts:
        ...
        if (maintype, subtype) in eml._body_types and not is_attachment(part) and subtype not in seen:
            seen.append(subtype)
            continue
        yield part
