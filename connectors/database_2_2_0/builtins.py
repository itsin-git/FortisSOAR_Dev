...
class DatabaseConnector():
    ...
    def make_query(self, query, *args, **kwargs):
        ...
        with self.engine.connect() as conn:
            q_results = conn.execute(query)
            is_fetch_query = q_results.returns_rows
            if is_fetch_query:
                results = q_results.fetchall()
        ...
        for res in results:
            row = {}
            index = 0
            for index, column in enumerate(q_results.keys()):
                row[column] = res[index]
            row_list.append(row)
        return json.loads(json.dumps(row_list, default=str))
