...
def python_inline(config, params):
    ...
    if config['allow_imports']:
        logger.debug('Universal import is allowed')
        import_pkg_list = []
        if allow_os_packages:
            import_pkg_list = ['os']
        try:
            exec(code_object, list_to_dict(import_pkg_list))
        ...
        if 'restrict_imports' in config:
            restricted_globals = list_to_dict(import_list(config['restrict_imports']))
            logger.debug(f"Inside restrict_imports, restricted_globals: {restricted_globals.keys()}")
            try:
                exec(code_object, restricted_globals)
                code_output = redirected_output.getvalue().replace('\n', '')
                return _parse(code_output, result)
            except (ParserError, YAMLError) as err:
                logger.error('A parser error has occurred: {}'.format(err))
...
def _parse(code_output, result):
    ...
    if code_output.startswith('{') and code_output.endswith('}'):
        ...
        if check_for_dict[-1] > 0:
            ...
            try:
                result['code_output'] = yaml.safe_load(code_output)
            except:
                result['code_output'] = yaml.safe_load(json.dumps(code_output))
                if result['code_output'].startswith('[') and result['code_output'].endswith(']'):
                    result['code_output'] = list(ast.literal_eval(result['code_output']))
                elif result['code_output'].startswith('{') and result['code_output'].endswith('}'):
                    result['code_output'] = dict(ast.literal_eval(result['code_output']))
    else:
        try:
            result['code_output'] = yaml.safe_load(code_output)
        except:
            ...
            if result['code_output'].startswith('[') and result['code_output'].endswith(']'):
                ...
                result['code_output'] = dict(ast.literal_eval(result['code_output']))
    logger.debug(f"Result output: {result['code_output']}")
    return result
...
def _check_health(config):
    ...
    if config['allow_imports']:
        ...
        list_to_import = import_list(config['restrict_imports'])
        logger.debug(f'list_to_import: {list_to_import}')
        if not allow_os_packages:
            if _regex_for_imports(list_to_import):
...
def _raise_error(traceback_str):
    msg_value = traceback_str[traceback_str.find('code_run') + len('code_run') + 3:]
    if 'NameError: name' in msg_value and 'is not defined' in msg_value:
        logger.debug(f'message value: {msg_value}')
        builtin_used = msg_value.split("'")[1::2]
        msg_value = f'Uses of {builtin_used} is restricted in the code snippet. Remove {builtin_used} from the code snippet and retry.'
    logger.error('Invalid code snippet: {}'.format(msg_value))
    raise ConnectorError('Invalid code snippet: {}'.format(msg_value))
