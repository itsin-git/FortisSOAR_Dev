        ...
        try:
            ...
            if response.ok:
                ...
                if isinstance(result.get('result'), list):
                    if result.get('result')[0].get('status').get('message') == "Login fail":
                        raise ConnectorError("Invalid credentials provided")
                ...
                if result.get('error'):
                    if "Invalid params" in result.get('error').get('message'):
                        raise ConnectorError("{0}".format(result.get('error')))
                    else:
...
def get_users(config, params):
    try:
        ...
        if params.get("fetch_type") == "Fetch All Users":
            payload = {
                "method": "get",
                "params": [
                    {
...
def get_endpoints(config, params):
    try:
        ...
        if params.get("fetch_type") == "Fetch All Endpoints":
            payload = {
                "method": "get",
                "params": [
                    {
...
def start_bulk_device_log_search_request(config, params):   
    try:    
        adom_name = params.get('adom_name') 
        start = params.get('start') 
        end = params.get('end') 
        obj = FortiAnalyzer(config) 
        obj.login() 
        result = build_payload(params, action_input_parameters.get('start_bulk_device_log_search_request')) 
        payload = { 
            "method": "add",    
            "params": [ 
                {   
        ...
        if result:
            if result.get('logtype'):
                result['logtype'] = log_type_mapping.get(result.get('logtype'))
            if result.get('devid'):    
                result['device'] = [{"devid": result.get('devid')}]
                result.pop('devid')
            if result.get('time-order'):    
                result['time-order'] = time_order_mapping.get(result.get('time-order')) 
                result.pop('time-order')    
...
def fetch_log_search_result_by_task_id(config, params):
    try:
        ...
        if response['result']['percentage'] == 100:  
            ...
            import time 
            time.sleep(15)  
            fetch_log_search_result_by_task_id(config, params)
    except Exception as err:
        logger.exception('{0}'.format(err))
        obj.logout()
        raise ConnectorError('{0}'.format(err))
...
def add_attachment(config, params):
    try:
        adom_name = params.get('adom_name')
        obj = FortiAnalyzer(config)
        obj.login()
        result = build_payload(params, action_input_parameters.get('add_attachment'))
        payload = {
            "method": "add",
            "params": [
                {
                    "url": "/incidentmgmt/adom/{adom_name}/attachments".format(adom_name=adom_name),
                    "attachments": [{"attachtype": params.get('attachtype'), "data": params.get('data')}],
                    "incid": params.get('incid'),
                    "apiver": APIVER
                }
            ],
        ...
        if result:
            result.pop('incid')
            result.pop('attachtype')
            result.pop('data')
            result.pop('adom_name')
            # payload.get('params')[0]['attachments'].append(result)
        response = obj.make_api_call(method='POST', json=payload)
        obj.logout()
        return response
    except Exception as err:
...
def get_log_status(config, params):
    try:
        ...
        if params.get('devid'):
            payload.get("params")[0].update({"device": [{"devid": result.get('devid')}]})
        response = obj.make_api_call(method='POST', json=payload)
        obj.logout()
        return response
    except Exception as err:
...
operations = {
    ...
    'start_bulk_device_log_search_request': start_bulk_device_log_search_request,
