...
def make_output(response, query_id=None, eventFlag=False):
    if eventFlag:
        ...
        result = {'events': [],
              '@totalCount': response['response']['result']['result']['@totalCount'],
              '@start': response['response']['result']['result']['@start'],
              '@queryId': query_id if query_id else ['response']['result']['result']['@queryId'],
              '@errorCode': response['response']['result']['result']['@errorCode']
              }        
    if 'queryResult' in response:
        ...
        if response['response']['result']['result']['@totalCount'] == '0':
            return result
        else:
            event_records = response['response']['result']['result']['events']['event']
