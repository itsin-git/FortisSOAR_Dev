...
def get_incidents(config, params):
    try:
        ...
        formatted_res = reformat_response(res)
        if "queryId" in formatted_res:
            return fetch_paginated_data(config, params, formatted_res)
        return formatted_res
    except Exception as err:
        raise ConnectorError(err)


def fetch_paginated_data(config, params, response):
    total_pages = response.get("pages", 0)
    incident_results = response.get("data", [])
    query_id = response.get("queryId")
    total_incidents = response.get("total")
    # In FortiSIEM 6.7.6 the pagination API is failing if number of pages is 2. This can be removed later.
    if total_pages == 2:
        params.update({"size": total_incidents})
    ...
    while page <= total_pages:
        endpoint = "/rest/pub/incident" + '/' + query_id + '/' + str(page)
        res = fortisiem_obj.make_rest_call(endpoint, headers=headers, method="GET")
        page += 1
        formatted_res = reformat_response(res)
        incident_results += formatted_res.get("data", [])
    response["data"] = incident_results
    return response
